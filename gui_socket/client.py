...
def start_socket(data_list):
    ...
    while check == 0:
        if data_list[8] != None:
            try:
                print("Trying to connect to " + str(CONNECT_IP) + ":" + str(CONNECT_PORT))
                s.connect((CONNECT_IP, int(CONNECT_PORT)))  # net stick
                print('Connection established. Retrieving MAVLink data & socket connection...')
                check = 1
            except Exception as e:
                print("Connection not established. With exception ", e)
                data_list[8] = None
                #exit(1)
    ...
    while True:
        ...
        if active:
            ...
            if len(ready_to_read) > 0:
                ...
                # mav_str = "DOWN"
                # cam_str = "DOWN"
            #print("ready_to_write len: " + str(len(ready_to_write)))
            if len(ready_to_write) > 0:
                dict['ID'] = str(msg_id)
                dict['LATENCY'] = data_list[0]
            ...
            try:
                ...
                s.connect((CONNECT_IP, int(CONNECT_PORT)))
            except OSError:
                time.sleep(1)
                continue
            print("Reconnected.")
...
# start_socket([0, "DOWN", "DOWN", 0, 0, "NOT ARMED", "", 0, '10.100.102.7', 12341])
