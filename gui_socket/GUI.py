...
def start_gui():
    ...
    def open_cam_web(ip, port):
        address = ip + ":" + port + "/stream"
    ...
    def conn_window():
        ...
        connect_button = tk.Button(conn_win, text="Connect", bg="lime green", command=lambda: print("Connecting..."))
    ...
    def update_inet_data(inet_lbl):
        ...
        inet_str = inet_delay + " ms"
        if (int(inet_delay) > 100):
    ...
    # gps latitude data
    gps_latitude_data = tk.Label(window, text="5.359699", fg="black", bg="white", width=12, font='Helvetica 9', relief="groove")
    gps_latitude_data.place(x=247, y=340)

    # gps latitude refresh button
    gps_latitude_ref_button = tk.Button(window, text="Refresh", bg="LightBlue3", command=None)
    gps_latitude_ref_button.place(x=347, y=337)
    ...
    # gps longitude data
    gps_longitude_data = tk.Label(window, text="-3.8874638", fg="black", bg="white", width=12, font='Helvetica 9', relief="groove")
    gps_longitude_data.place(x=247, y=370)

    # gps longitude refresh button
    gps_longitude_ref_button = tk.Button(window, text="Refresh", bg="LightBlue3", command=None)
    gps_longitude_ref_button.place(x=347, y=367)


    # copy gps longitude to clipboard
    def copy_lat_clipboard():
        lat_to_copy = gps_longitude_data['text']
        window.clipboard_clear()
        window.clipboard_append(lat_to_copy)


    # gps longitude copy button
    gps_longitude_copy_button = tk.Button(window, text="Copy to Clipboard", bg="LightBlue3", command=copy_lat_clipboard)
    ...
    def update_armed_data(armed_lbl):
        ...
        armed_lbl.config(text=armed_str)
        armed_lbl.after(1000, lambda: update_mav_data(armed_lbl))
